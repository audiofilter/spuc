...
fast_equalizer::fast_equalizer(int BANDS) : 
  ...
  eqh(BANDS),
  eql(BANDS),
  fil_out(BANDS-1),eq(BANDS),levels(BANDS),gains(BANDS)
{ 
  num_bands = BANDS;
  reset();
  lev_gain = 0.75;
    ...
    void fast_equalizer::set_num_bands(int B) {
        ...
        if (B<MIN_BANDS) num_bands = MIN_BANDS; // don't bother < 3
        else num_bands = B;
    }
...
bool fast_equalizer::stage(complex<double> s, int i,
						   complex<double>& h,
						   complex<double>& l) {
  // Do HbF
  bool vld1 = vld_All[i];
...
complex<double> fast_equalizer::stage_up_down(int i,
											  complex<double> s) 
{
  complex<double> up;
  complex<double> up2;
  if (vld_en[i-1]) {
	...
	if (i<num_bands-2) tl2 = stage_up_down(i+1,tl2);
	else tl2 = gains[i+1]*tl2;
	up2 = tl2 + gains[i]*th2;
  } else {
	up2 = 0;
  }
