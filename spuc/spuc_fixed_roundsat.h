...
template <int TOTAL_BITS_, int FRAC_BITS_, spuc_q_mode SPUC_Q_MODE_, spuc_o_mode SPUC_O_MODE_> class spuc_fixed;
template <int TOTAL_BITS_, int FRAC_BITS_, spuc_q_mode SPUC_Q_MODE_, spuc_o_mode SPUC_O_MODE_> class spuc_ufixed;
#endif
...
template <> class spuc_round_class_function<SPUC_RND> {
 public:
  ...
  template <typename T_, typename TS_> static T_ round(const T_ &a, const TS_ &shift) {
    T_ x = a >> shift;
...
template <int FRAC_BITS_, spuc_q_mode Q_> class spuc_shift_class_function {
 public:
  ...
  shift_function(const spuc_fixed<TOTAL_BITS_1, INT_BITS_1, SPUC_Q_MODE_1, SPUC_O_MODE_1> &a) {
    ...
    } else { x = (a.getVal() << (int)Abs_Func<TOTAL_BITS_1 - INT_BITS_1 - FRAC_BITS_>::val); }
    return (x);
  }
  template <int TOTAL_BITS_1, int INT_BITS_1, spuc_q_mode SPUC_Q_MODE_1, spuc_o_mode SPUC_O_MODE_1>
  static typename int_size_needed<INT_BITS_1 + Template_Max<TOTAL_BITS_1 - INT_BITS_1, FRAC_BITS_>::maxval>::int_type
...
template <int FRAC_BITS_> class spuc_shift_class_function<FRAC_BITS_, SPUC_RND> {
 public:
...
template <int FRAC_BITS_> class spuc_shift_class_function<FRAC_BITS_, SPUC_TRN> {
 public:
...
/// fixed-point types
template <int TOTAL_BITS_, spuc_o_mode SPUC_O_MODE_> class spuc_saturate_class_function {
 public:
  ...
  static typename int_size_needed<TOTAL_BITS_>::int_type saturate(const max_int_type &a) {
    typename int_size_needed<TOTAL_BITS_>::int_type x;
  ...
  static typename int_size_needed<TOTAL_BITS_>::int_type usaturate(const max_int_type &a) {
    typename int_size_needed<TOTAL_BITS_>::int_type x;
...
template <int TOTAL_BITS_> class spuc_saturate_class_function<TOTAL_BITS_, SPUC_SAT> {
 public:
  ...
  static typename int_size_needed<TOTAL_BITS_>::int_type saturate_function(
    ...
    x = (a.getVal() > ((max_int_type(1) << (TOTAL_BITS_ - 1)) - 1))
            ...
            : (a.getVal() < ((max_int_type(-1) << (TOTAL_BITS_ - 1)))) ? ((max_int_type(-1) << (TOTAL_BITS_ - 1)))
                                                                       : a.getVal();
  ...
  static typename int_size_needed<TOTAL_BITS_>::int_type saturate_function(
      const spuc_ufixed<TOTAL_BITS_1, INT_BITS_1, SPUC_Q_MODE_1, SPUC_O_MODE_1> &a) {
    typename int_size_needed<TOTAL_BITS_>::int_type x;
    x = (a.getVal() > ((max_int_type(1) << (TOTAL_BITS_)) - 1)) ? ((max_int_type(1) << (TOTAL_BITS_)) - 1) : a.getVal();
    return (x);
  }
  static typename int_size_needed<TOTAL_BITS_>::int_type saturate(const max_int_type &a) {
    typename int_size_needed<TOTAL_BITS_>::int_type x;
    x = (a > ((max_int_type(1) << (TOTAL_BITS_ - 1)) - 1))
            ? ((max_int_type(1) << (TOTAL_BITS_ - 1)) - 1)
            : (a < ((max_int_type(-1) << (TOTAL_BITS_ - 1)))) ? ((max_int_type(-1) << (TOTAL_BITS_ - 1))) : a;
    //					std::cout << "\n saturate_func " << (int)a << " -> " <<
    //(int)x
    //<< "\n";
    return (x);
  }
  static typename int_size_needed<TOTAL_BITS_>::uint_type usaturate(const max_int_type &a) {
    typename int_size_needed<TOTAL_BITS_>::uint_type x;
    x = (a > ((max_int_type(1) << TOTAL_BITS_) - 1)) ? ((max_int_type(1) << TOTAL_BITS_) - 1) : (a < 0) ? 0 : a;
    return (x);
  }
};
/// Specialized for SPUC_SAT_SYM
template <int TOTAL_BITS_> class spuc_saturate_class_function<TOTAL_BITS_, SPUC_SAT_SYM> {
 public:
  ...
  static typename int_size_needed<TOTAL_BITS_>::int_type saturate(const max_int_type &a) {
    ...
    x = (a > ((max_int_type(1) << (TOTAL_BITS_ - 1)) - 1))
            ? ((max_int_type(1) << (TOTAL_BITS_ - 1)) - 1)
            : (a < ((max_int_type(-1) << (TOTAL_BITS_ - 1)) + 1)) ? ((max_int_type(-1) << (TOTAL_BITS_ - 1)) + 1) : a;
    //					std::cout << "\n saturate_func " << (int)a << " -> " <<
    //(int)x
    //<< "\n";
    return (x);
