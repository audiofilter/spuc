...
//////////////////////////////////////////////////////////////////////////////
#ifndef SPUC_FIXED_20140509_CLASS_FUNCTIONS_H_
#define SPUC_FIXED_20140509_CLASS_FUNCTIONS_H_
//////////////////////////////////////////////////////////////////////////////
...
std::ostream &operator<<(std::ostream &os, const SPUC::spuc_fixed<NINT, NFRC, SPUC_Q_MODE, SPUC_O_MODE> &r) {
  return os << r.to_double();  // << " ";
};
template <int NINT, int NFRC, SPUC::spuc_q_mode SPUC_Q_MODE, SPUC::spuc_o_mode SPUC_O_MODE>
std::istream &operator>>(std::istream &os, const SPUC::spuc_fixed<NINT, NFRC, SPUC_Q_MODE, SPUC_O_MODE> &r) {
  return os >> r;
};
...
template <int TOTAL_BITS_, int INT_BITS_, int TOTAL_BITS_1, int INT_BITS_1, spuc_q_mode QMODE, spuc_q_mode QMODE1,
          spuc_o_mode OMODE, spuc_o_mode OMODE1>
spuc_fixed<Template_Max_Total_Bits<TOTAL_BITS_, INT_BITS_, TOTAL_BITS_1, INT_BITS_1>::maxval,
...
spuc_fixed<TOTAL_BITS_ + W, INT_BITS_ + W, QMODE, OMODE> operator<<(
    const spuc_fixed<TOTAL_BITS_, INT_BITS_, QMODE, OMODE> &a, const spuc_uint<W> &shift) {
  spuc_fixed<TOTAL_BITS_ + W, INT_BITS_ + W, QMODE, OMODE> tmp(a);
  tmp <<= shift;
  return (tmp);
}
// <<
template <int W, int TOTAL_BITS_, int INT_BITS_, spuc_q_mode QMODE, spuc_o_mode OMODE>
spuc_fixed<TOTAL_BITS_ + W, INT_BITS_ + W, QMODE, OMODE> operator<<(
    const spuc_fixed<TOTAL_BITS_, INT_BITS_, QMODE, OMODE> &a, const spuc_int<W> &shift) {
  spuc_fixed<TOTAL_BITS_ + W, INT_BITS_ + W, QMODE, OMODE> tmp(a);
  tmp <<= shift;
  return (tmp);
}
...
spuc_fixed<TOTAL_BITS_, INT_BITS_, QMODE, OMODE> operator>>(const spuc_fixed<TOTAL_BITS_, INT_BITS_, QMODE, OMODE> &a,
                                                            const int &shift) {
  spuc_fixed<TOTAL_BITS_, INT_BITS_, QMODE, OMODE> tmp(a);
  tmp >>= shift;
  return (tmp);
}
// << with int
...
// spuc_fixed<TOTAL_BITS_,INT_BITS_,QMODE_,OMODE_> &b)  {return (b.to_int() ==
//(int)a);	}
