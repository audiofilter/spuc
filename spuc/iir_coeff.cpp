...
namespace SPUC {
iir_coeff::iir_coeff(long ord) : poles((ord+1)/2), zeros((ord+1)/2),
								 a_tf(ord+1), b_tf(ord+1)
{
...
float_type iir_coeff::get_coeff_a(long i) { 
  if (i<order) {
	if (i%2 == 0) return(real(poles[i/2]));
	else          return(imag(poles[i/2]));
  } else {
	return(0);
  }
}
float_type iir_coeff::get_coeff_b(long i) { 
  if (i<order) {
	if (i%2 == 0) return(real(zeros[i/2]));
	else          return(imag(zeros[i/2]));
  } else {
	return(0);
  }
}
...
complex<float_type> iir_coeff::get_root(long i) {
  if (i <= n2) return(poles[i]);
  else return(complex<float_type>(0.0,0.0));
}
complex<float_type> iir_coeff::get_zero(long i) {
  if (i <= n2) return(zeros[i]);
  else return(complex<float_type>(0.0,0.0));
}
void iir_coeff::pz_to_ap() {
  int m = 2*order-1;
  typedef smart_array<float_type> Array;
  typedef smart_array<complex<float_type> > CArray;
  Array fa;
  Array d2(m);
  Array p2(m);
  Array r(m);
