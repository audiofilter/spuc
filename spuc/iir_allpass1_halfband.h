...
template <class Numeric,class Coeff=float_type> class iir_allpass1_halfband
{
 ...
 public:
  iir_allpass1_halfband(Coeff c0, Coeff c1, long round_bits=0)
    : A0(c0,1,round_bits), A1(c1,1,round_bits) { even = 0;	}
  //! reset
  void reset() { A0.reset(); A1.reset(); out0=out1=(Numeric)0; }
  ...
  Numeric clock(Numeric input) {
	...
	even = !even;
	if (even) return(round((out0 + out1),1));
	else return((Numeric)0); // to indicate that this sample is not calculated
