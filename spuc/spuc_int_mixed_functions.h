...
spuc_int<Template_Max<W_, W_1 + 1>::maxval + 1> operator+(
    const spuc_int<W_> &a, const spuc_uint<W_1> &b) {
  spuc_int<Template_Max<W_, W_1 + 1>::maxval + 1> tmp(a);
  tmp += b;
  return (tmp);
}
template <int W_, int W_1>
spuc_int<Template_Max<W_ + 1, W_1>::maxval + 1> operator+(
    const spuc_uint<W_> &a, const spuc_int<W_1> &b) {
