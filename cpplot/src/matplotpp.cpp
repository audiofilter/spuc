...
void MatPlot::display(){
	...
	if(mode==1){// Configuration of objects
	    ...
	    for(size_t i=0;i<vAxes.size();++i){ vAxes[i].config(); }
...
void MatPlot::keyboard(unsigned char key){
    switch(key){
    case 'q':
	exit(0);
	break;
...
void MatPlot::display_layer(){
    ...
    int tObj;// type  of child object
    int iObj;// index of child object	
    for(size_t i=0;i<cfr->Children.size();++i){
...
void MatPlot::display_layer2(){
	...
	for(size_t j=0;j<vLayer.size();++j){
...
void MatPlot::Layer_mouse(int button, int state, int x, int y ){
	...
	for(size_t j=0;j<vLayer.size();++j){
	  if((l<x)&&(x<w)&&(h*(int)j<y)&&(y<h*(int)j+h)){
		...
		if(dt < 0.05 ){
		    for(size_t k=0;k<vLayer.size();++k){ vLayer[k].Visible=0; }
		    vLayer[j].Visible=1;
		    //cout <<"!!"<<endl;
		}
...
void MatPlot::display_axes_2d(){
	...
	if(ca->XGrid){
	    glLineWidth(ca->LineWidth);
	    gl2psLineWidth(ca->LineWidth);
	    for(size_t i=0;i<ca->XTick.size();++i){
	...
	// y tick 
	for(size_t i=0;i<ca->YTick.size();++i){
	    glBegin(GL_LINE_STRIP);
	    glVertex2d( l,             cty2(ca->YTick[i]) );
...
void MatPlot::display_axes_3d(){
    ...
    */
	...
	if(ca->TickLabel){
	    ...
	    // z
	    for(size_t i=0;i<ca->ZTick.size();++i){
...
int MatPlot::plot(valarray<double> x,valarray<double> y){	
    dvec xx,yy;
    for(size_t i=0;i<x.size();++i){xx.push_back(x[i]);}
    for(size_t i=0;i<y.size();++i){yy.push_back(y[i]);}    
    return line(xx,yy);
}
...
int MatPlot::errorbar(dvec x,dvec y,dvec e){	
	begin();	
	for(size_t i=0;i<x.size();++i){ vertex(x[i],y[i],e[i],e[i]); }
	end();
	cl->Errorbar=1;
	return 0;
    }
int MatPlot::errorbar(dvec x,dvec y,dvec ep,dvec em){
	begin();
	for(size_t i=0;i<x.size();++i){ vertex(x[i],y[i],ep[i],em[i]); }
	end();
	cl->Errorbar=1;
	return 0;
    }
...
int MatPlot::plot3(dvec x,dvec y,dvec z){
    ca->View=1;
    begin();
    for(size_t i=0;i<x.size();++i){ vertex(x[i],y[i],z[i]); }
    end();
    return 0;
}
...
void MatPlot::display_line(){
	...
	if(cl->LineStyle !="none" ){// Line //
	    ...
	    if(cl->XData.size()){
		glBegin(GL_LINE_STRIP);
		for(size_t i=0;i<cl->XData.size();++i){
		    //printf("i:%d %f %f\n",i,xx,yy);
		    xx=ctx(cl->XData[i]);
		    yy=cty(cl->YData[i]);
		    glVertex2d(xx,yy);
	...
	if(is_print){
	  printf("vc.size %d\n",(int)vc.size());
	    for(size_t k=0;k<vc.size();++k){
		  printf("vc: %2d : %2d %2d :%f %f\n",(int)k,vc[k].xj,vc[k].yi,vc[k].x,vc[k].y);
	    }
	}
	// sort contour points    
	int is;
